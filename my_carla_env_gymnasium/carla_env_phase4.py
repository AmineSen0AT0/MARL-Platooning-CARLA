...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    observation_space_dict = {
      ...
      # FIX 2: Relaxed bounds for 'state' to prevent crashes during training
      # Original was [-2, 2], we change to [-inf, inf] for safety
      'state': spaces.Box(np.array([-np.inf, -np.inf, -np.inf, -np.inf, -np.inf]),
  ...
  def _get_obs(self):
    ...
    # ==============================================================================
    # FIX: MULTI-AGENT STATE MATRIX LOOP WITH V2V COMMUNICATION (AIR 2026 NOVELTY)
    # ==============================================================================
    # We upgrade the array from 4 to 5 dimensions to hold the V2V broadcast
    state_matrix = np.zeros((self.num_agents, 5), dtype=np.float32)

    # V2V BROADCAST: Extract the Pace Car's exact speed before the loop
    leader_v = self.egos[0].get_velocity()
    leader_speed = np.sqrt(leader_v.x ** 2 + leader_v.y ** 2)
    ...
    for i in range(self.num_agents):
      ...
      # THE NOVELTY: Injecting 'leader_speed' directly into everyone's brain!
      # Agent 5 now knows what the Pace Car is doing instantly, bypassing the physical shockwave.
      state_matrix[i] = [lateral_dis, - delta_yaw, speed, radar_danger, leader_speed]
    # ==============================================================================
  ...
  def _get_reward(self):
    """Calculate the Cooperative MARL step reward for Shockwave Mitigation."""

    # We will collect the individual rewards of the 5 followers
    follower_rewards = []

    # Agent 0 is the Pace Car. It does not learn, so we skip it (start at 1).
    for i in range(1, self.num_agents):
      ego_loc = self.egos[i].get_transform().location
      front_loc = self.egos[i - 1].get_transform().location

      # Calculate exact physical distance to the car ahead
      dist_to_leader = np.sqrt((front_loc.x - ego_loc.x) ** 2 + (front_loc.y - ego_loc.y) ** 2) - 2.5
      dist_to_leader = min(dist_to_leader, 50.0)
      v = self.egos[i].get_velocity()
      speed = np.sqrt(v.x ** 2 + v.y ** 2)
      control = self.egos[i].get_control()

      # 1. DISTANCE TRACKING PENALTY (Target is exactly 12.0 meters)
      # If they are at 12m, penalty is 0. If they are at 5m or 20m, penalty is large.
      dist_error = abs(dist_to_leader - 12.0)
      r_dist = - (dist_error ** 2) * 0.05

      # 2. DANGER PENALTY (Impending Crash)
      r_danger = 0.0
      if dist_to_leader < 4.0:
        r_danger = -50.0  # Massive penalty for getting dangerously close

      # 3. JERK / COMFORT PENALTY (Curing the Shockwave)
      # We heavily penalize the brake pedal. This forces the MARL to use V2V
      # to predict the jam and lift off the gas early instead of slamming the brakes.
      r_jerk = 0.0
      if control.brake > 0.0:
        r_jerk = - (control.brake ** 2) * 10.0

      # 4. FORWARD PROGRESS REWARD
      # Small reward just for keeping the platoon moving, preventing them from just parking
      r_speed = speed * 0.5

      # Sum the metrics for this specific agent
      agent_reward = r_dist + r_danger + r_jerk + r_speed
      follower_rewards.append(agent_reward)

    # ==============================================================================
    # COOPERATIVE GLOBAL REWARD (AIR 2026 NOVELTY)
    # We average the scores of all 5 followers. If Agent 5 suffers, Agent 1 suffers.
    # ==============================================================================
    global_reward = sum(follower_rewards) / len(follower_rewards)

    # Finally, if ANY physical collision is detected by the physical collision sensor (e.g. Pace Car hitting a wall)
    if len(self.collision_hist) > 0:
      global_reward -= 50000.0  # Episode-ending disaster

    # NEW: VIRTUAL COLLISION DETECTOR (For the followers rear-ending each other)
    # This completely destroys the "Suicide Exploit" loophole!
    for i in range(1, self.num_agents):
      front_loc = self.egos[i - 1].get_transform().location
      ego_loc = self.egos[i].get_transform().location

      # Pure center-to-center distance, exactly matching the _terminal() function
      center_dist = np.sqrt((front_loc.x - ego_loc.x) ** 2 + (front_loc.y - ego_loc.y) ** 2)

      if center_dist <= 5.5:
        global_reward -= 50000.0
        break  # Apply the penalty once and break the loop so it doesn't double-charge
  ...
  #   global_reward = sum(follower_rewards) / len(follower_rewards)
  #
  #   # Finally, if ANY physical collision is detected by the collision sensor
  #   if len(self.collision_hist) > 0:
  #     global_reward -= 50000.0  # Episode-ending disaster
  #
  #   return global_reward
  ...
  def _terminal(self):
    """Calculate whether to terminate the current episode."""
    # 1. If the Platoon Leader collides with a wall/npc (Sensor is on Agent 0)
    if len(self.collision_hist) > 0:
      return True

    # 2. Loop through all 6 agents to check for lane departures or rear-end crashes
    for i in range(self.num_agents):
      ...
      # If ANY of the 6 cars goes out of lane, terminate the episode
      dis, _ = get_lane_dis(self.waypoints_list[i], ego_x, ego_y)
      if abs(dis) > self.out_lane_thres:
        return True

      # 3. FIXED: VIRTUAL COLLISION SENSOR FOR THE LINCOLN MKZ
      # The Lincoln is ~5.0 meters long. If center-to-center distance is < 5.5m, it is a crash!
      if i > 0:
        front_loc = self.egos[i - 1].get_transform().location
        ego_loc = self.egos[i].get_transform().location

        # Calculate pure center-to-center Euclidean distance
        center_dist = np.sqrt((front_loc.x - ego_loc.x) ** 2 + (front_loc.y - ego_loc.y) ** 2)

        # Trigger the crash if they touch!
        if center_dist <= 5.5:
          return True
  ...
  #       return True
  #
  #     # 3. NEW: VIRTUAL COLLISION SENSOR FOR FOLLOWERS
  #     # If any follower rear-ends the car in front of it, terminate immediately!
  #     if i > 0:
  #       front_loc = self.egos[i - 1].get_transform().location
  #       ego_loc = self.egos[i].get_transform().location
  #
  #       # Distance formula minus 2.5 meters for car length
  #       front_dist = np.sqrt((front_loc.x - ego_loc.x) ** 2 + (front_loc.y - ego_loc.y) ** 2) - 2.5
  #
  #       # If the gap drops below 0.5 meters, they physically crashed
  #       if front_dist <= 0.5:
  #         return True
  #
  #   return False
