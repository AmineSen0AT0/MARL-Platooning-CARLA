...
class RoutePlanner():
  ...
  def _compute_next_waypoints(self, k=1):
    ...
    for _ in range(k):
      ...
      raw_next_waypoints = list(last_waypoint.next(self._sampling_radius))

      if len(raw_next_waypoints) == 1:
        # only one option available ==> lanefollowing
        next_waypoint = raw_next_waypoints[0]
        road_option = RoadOption.LANEFOLLOW
      else:
        # 1. Sort by map ID to defeat C++ memory scrambling
        next_waypoints = sorted(raw_next_waypoints, key=lambda wp: (wp.road_id, wp.lane_id))
        road_options_list = retrieve_options(next_waypoints, last_waypoint)

        # ==============================================================================
        # 2. SPATIAL SEEDING: Lock the dice using the asphalt's ID!
        # This guarantees that no matter WHEN a car arrives at this intersection,
        # it will roll the exact same random choice as the Pace Car did.
        # ==============================================================================
        intersection_id = last_waypoint.road_id * 1000 + abs(last_waypoint.lane_id)
        random.seed(intersection_id)

        road_option = random.choice(road_options_list)
        next_waypoint = next_waypoints[road_options_list.index(road_option)]

        # 3. Unfreeze the dice immediately so we don't break other systems
        random.seed()
  ...
  def _is_vehicle_hazard(self, vehicle_list):
    """
    UPGRADED DISTANCE RADAR:
    Returns the exact distance (float) to the closest vehicle directly in front of us
    in the same lane. Maximum radar range is 50.0 meters.
    """
    ego_vehicle_location = self._vehicle.get_location()
    ego_vehicle_waypoint = self._map.get_waypoint(ego_vehicle_location)

    # We set our maximum radar vision to 50 meters.
    # If no car is found, the radar returns 50.0 (meaning "road is completely clear")
    ...
    for target_vehicle in vehicle_list:
      ...
      # if the object is not in our lane it's not an obstacle (Lane Filtering!)
      ...
      # Check up to 50 meters ahead instead of the old 15-meter proximity threshold
      if is_within_distance_ahead(loc, ego_vehicle_location,
                                  self._vehicle.get_transform().rotation.yaw,
                                  50.0):
        # Calculate the exact distance
        dist = ego_vehicle_location.distance(loc)
